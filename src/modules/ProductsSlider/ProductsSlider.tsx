...
const resolveProductsByPreset = (
  ...
  switch (preset) {
    ...
    case ProductsPresetType.HotPrices:
      return sortProducts(filtered, SortType.HotPrices).slice(0, limit);
...
export const ProductsSlider = ({
  ...
  }, [productsPresetType, category, excludeItemId, limit]);
  ...
  return (
    <section
      className={classNames(`${styles.productSlider} ${className || ''}`)}
    >
      <div className={styles.header}>
        <h2 className={styles.title}>{title}</h2>

        <div className={styles.buttonBox}>
          <ArrowButton onClick={prev} arrowClassName={styles.arrowLeft} />
          <ArrowButton onClick={next} arrowClassName={styles.arrowRight} />
        </div>
      </div>
